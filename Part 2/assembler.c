...
int main()	{
	...
	char opcode[50][2][40]; //50 is the length of String and Its a 2D array of Strings.
	char regis[50][2][40];
	char label[50][2][40];
	char st[100];
	...
	do	{										//writing binary representation of assembly code.
		...
		len=strlen(st);
		if(st[len-1]==':')	
		{					//skip if label instruction.(leaving label line blank in binary file)
			//fprintf(bin," ");
			stlen=0;
		}
		else	
		{
			for(j=0;j<opj;j++)						//comparing string from file and opcode array and then writing binary form in file
				...
				{
				for(j=0;j<regj;j++)	
				{				//comparing string from file and register array and then writing binary form in file
			...
			{
				for(j=0;j<symj;j++)	
				{									//reading line number from symbol table file and converting it to binary and then writing in binary file
					if(strcmp(st,label[j][0])==0)	
			...
			if(fl==0 && strcmp(st,";")!=0)	
			{		//when immediate values are encountered in assembly code
